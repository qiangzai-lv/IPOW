...
class FasterITOW(TwoStageDetector):
    ...
    def __init__(self,
        ...
        # text embedding
        self.known_texts = torch.load(self.owod_cfg['known_text_embeddings_path'])['texts']
        ...
        self.gmm_sampled_boxes = sample_all_categories_from_model(self.gmm_models,
                                                                  num_per_class=self.owod_cfg.gmm_n_samples)
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # ROI
        roi_losses = self.roi_head.loss(x, all_text_embedding, [self.con_concept_embeddings, self.dist_concept_embeddings], gmm_rpn_results,
                                        fix_anchors_list,
                                        batch_data_samples)
    ...
    def _forward(self, batch_inputs: Tensor,
        ...
        gmm_boxes_tensor = torch.tensor(self.gmm_sampled_boxes, dtype=bbox_dtype, device=device)
        gmm_labels = torch.zeros(len(gmm_boxes_tensor), dtype=label_dtype, device=device)
        gmm_scores = torch.full((len(gmm_boxes_tensor),), 1.0, dtype=score_dtype, device=device)
        ...
        roi_outs = self.roi_head.forward(x, [self.con_concept_embeddings, self.dist_concept_embeddings],
                                         gmm_rpn_results,
                                         fix_anchors_list,
                                         batch_data_samples)
