...
from mmdet.datasets.owodb_const import *
        ...
        with tempfile.TemporaryDirectory(prefix="pascal_voc_eval_") as dirname:
            ...
            for cls_id, cls_name in enumerate(self._class_names):
                ...
                if cls_id < self.num_seen_classes:
                    self._logger.info(f"{cls_name:40s}[{cls_id:02d}]: AP{thresh}={aps[thresh][-1]:.3f}, #pred={len(lines)}")
                else:
                    self._logger.info(f"{cls_name:40s}[{cls_id:02d}]: AR{thresh}={recs[thresh][-1]:.3f}, #pred={len(lines)}")
